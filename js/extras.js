...
(function () {
  ...
  function toRiyadhTime(dateStr, timeStr){
    const [y,m,d]=dateStr.split('-').map(Number);
    const [hh,mm]=timeStr.split(':').map(Number);
    const dt=new Date(Date.UTC(y,m-1,d,hh+7,mm));
    return dt.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',hour12:false})+' AST';
  }
    ...
    list.innerHTML = `
      ...
      ${items.map(m => `
        <div class="match-row">
          ...
          <div class="match-vs">
            <span class="vs-time">${toRiyadhTime(m.date,m.time)}</span>
